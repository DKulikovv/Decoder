...
typedef struct
{
  ...
  int keepEnglishLetter;         // says if this script contains English characters (not implemented)
...
public:
  ...
  nsSingleByteCharSetProber(SequenceModel *model, int reversed, nsCharSetProber* nameProber)
    :mModel(model), mReversed(reversed), mNameProber(nameProber) { Reset(); }

  virtual const char* GetCharSetName();
  virtual nsProbingState HandleData(const char* aBuf, unsigned aLen);
  ...
  int KeepEnglishLetters() {return mModel->keepEnglishLetter;}; // (not implemented)
...
protected:
  ...
  const int mReversed; // PR_TRUE if we need to reverse every pair in the model lookup

  //char order of last character
  unsigned char mLastOrder;

  unsigned mTotalSeqs;
  unsigned mSeqCounters[NUMBER_OF_SEQ_CAT];
