...
nsMBCSGroupProber::nsMBCSGroupProber(unsigned aLanguageFilter)
{
  for (unsigned i = 0; i < NUM_OF_PROBERS; i++)
    mProbers[i] = nsnull;
...
nsProbingState nsMBCSGroupProber::HandleData(const char* aBuf, unsigned aLen)
{
  nsProbingState st;
  unsigned start = 0;
  unsigned keepNext = mKeepNext;

  //do filtering to reduce load to probers
  for (unsigned pos = 0; pos < aLen; ++pos)
  {
    if (aBuf[pos] & 0x80)
    {
      if (!keepNext)
...
#ifdef DEBUG_jgmyers
void nsMBCSGroupProber::GetDetectorState(nsUniversalDetector::DetectorState (&states)[nsUniversalDetector::NumDetectors], unsigned &offset)
{
  for (unsigned i = 0; i < NUM_OF_PROBERS; ++i) {
