...
public:
  ...
  void HandleData(const char* aBuf, unsigned aLen);

  void HandleOneChar(const char* aStr, unsigned aCharLen)
  {
    int order;
  ...
  void      Reset(int aIsPreferredLanguage);
  void      SetOpion(){};
  int GotEnoughData() {return mTotalRel > ENOUGH_REL_THRESHOLD;};

protected:
  virtual int GetOrder(const char* str, unsigned *charLen) = 0;
  virtual int GetOrder(const char* str) = 0;

  //category counters, each interger counts sequence in its category
  unsigned mRelSample[NUM_OF_CATEGORY];

  //total sequence received
  unsigned mTotalRel;

  //Number of sequences needed to trigger detection
  unsigned mDataThreshold;
...
class SJISContextAnalysis : public JapaneseContextAnalysis
{
  //SJISContextAnalysis(){};
protected:
  int GetOrder(const char* str, unsigned *charLen);
